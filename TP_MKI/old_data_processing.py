...
class DataProcessing:
    ...
    def prepare_data(
        ...
        for f in past_data["frames"][:5]:
            print("frame", f["frame_index"], "bbox", f.get("bbox"), "landmarks", f.get("pose_landmarks") is not None)
        future_data = {"frames": data["frames"][new_total_frames:]}

        X = self.feature_extraction(past_data, use_bbox=use_bbox, use_skeleton=use_skeleton,
                                    use_angles=use_angles,
                                    use_reduced_skeleton=use_reduced_skeleton)
        y = self.feature_extraction(future_data, use_bbox=use_bbox, use_skeleton=use_skeleton,
                                    use_angles=use_angles,
                                    use_reduced_skeleton=use_reduced_skeleton)
    ...
    def build_features(self, frames, use_bbox=True, use_skeleton=True, use_angles=True, selected_landmarks=[0,11,12,23,24]):
        ...
        for f in frames:
            ...
            if use_skeleton:
                ...
                if lm is None:
                    print(f"Frame {f.get('frame_index')} has no skeleton")
